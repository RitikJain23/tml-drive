...
export class ShareObjectModal extends React.Component {
  ...
  urlShortner(url){
    //console.log("HIIIIIIIWIIWIWIWIWIW "+configs.urlShortnerKey);
    const options={
      "dynamicLinkInfo": {
        "dynamicLinkDomain": "tmldrive.page.link",
        "link": url
      },
      "suffix": {
        "option": "SHORT" 
    ...
    axios.post("https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key="+configs.urlShortnerKey,options).then(response => {
    // this.shortURL = "fff";
    // console.log("URL "+ this.shortURL);
    // console.log("tradhsjd "+response.data.shortLink)
    this.setState({
      short_url:response.data.shortLink
    })
